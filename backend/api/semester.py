...
async def _run_end_of_semester(conn):
    ...
    for cls in active_classes:
        conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, 0)
        """, (cls['course_id'], cls['instructor_id'], new_semester, 
              cls['class_time'], cls['capacity'], cls['avg_rating']))
    ...
    for student in students:
        ...
        if grades:
            ...
            if all_grades:
                total = sum(grade_points.get(g['grade'], 0) for g in all_grades)
    ...
    print(f"✅ End of semester {current_semester} processed. Started semester {new_semester}")
